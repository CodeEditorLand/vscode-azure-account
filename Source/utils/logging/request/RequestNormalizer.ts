...
import type * as request from "request-promise";
import type {
	HttpNormalizer,
...
export class RequestNormalizer
	implements HttpNormalizer<request.Options, RequestResponseInfo>
{
	...
	normalizeResponse(info: RequestResponseInfo): NormalizedHttpResponse {
		return {
			...
			bodyAsText:
				"body" in info.response
					? JSON.stringify(info.response.body)
					: undefined,
		};
	}
}
