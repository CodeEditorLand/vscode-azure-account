...
import type { AzureAccountLoginHelper } from "./AzureAccountLoginHelper";
import {
	type SettingsCache,
	type SettingsCacheVerified,
	cachedSettingKeys,
	settingsCacheKey,
} from "./SettingsCache";
...
export async function checkSettingsOnStartup(
	...
	let shouldSignOutAndReload = false;
	...
	for (let index = 0; index < numSettings; index++) {
		...
		if (
			lastSeenSettingValue &&
			settingValueOnStartup !== lastSeenSettingValue
		) {
...
export async function askThenSignOutAndReload(
	...
	await window
		.showInformationMessage(signOutAndReloadRequired, signOutAndReload)
		.then(async (value) => {
...
export async function askForSignIn(
	...
	// Purposefully await this message to block whatever command caused the extension to activate.
	await window
		.showInformationMessage(signInAgainRequired, signIn)
		.then(async (value) => {
			if (value === signIn) {
...
function addPropertyToTelemetry(
	actionContext: IActionContext,
	eventPrefix: string,
	eventSuffix: string,
