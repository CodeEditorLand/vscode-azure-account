...
export class AdalAuthProvider extends AuthProviderBase<TokenResponse[]> {
	...
	public async loginWithDeviceCode(
	...
	public async loginSilent(
		...
		if (parsedCreds) {
			...
			tokenResponse = await getTokenWithAuthorizationCode(
				clientId,
	...
	public getCredentials(
		...
		return new DeviceTokenCredentials({
			...
			username: userId,
			clientId,
			tokenCache: this.delayedTokenCache,
			domain: tenantId,
		});
	}
	...
	public async updateSessions(
		...
		sessions.splice(
			0,
			sessions.length,
			...loginResult.map<AzureSession>((tokenResponse) => ({
				environment,
	...
	public async clearTokenCache(): Promise<void> {
		...
		const allEnvironmentNames: string[] = staticEnvironmentNames.concat([
			"Azure",
			"AzureChina",
		]);
